...
def scrape_ytd_from_morningstar_api(url,headers):
	try:
		logging.info(f"Fetching YTD from Morningstar API: {url}");response=requests.get(url,headers=headers,timeout=15);response.raise_for_status();data=response.json();growth_data=data.get('table',{}).get('growth10KReturnData',[]);fund_data=next((item for item in growth_data if item.get('label')=='fund'),None)
...
def scrape_inception_return_from_api(url,headers):
	try:
		logging.info(f"Fetching Inception Return from API: {url}");response=requests.get(url,headers=headers,timeout=15);response.raise_for_status();data=response.json();net_return_list=data.get('netReturn',[])
		if net_return_list:
...
def scrape_instrument_data(instrument_url,css_selector):
	try:
		logging.info(f"Fetching URL: {instrument_url}");response=requests.get(instrument_url,headers=HEADERS,timeout=15);response.raise_for_status();soup=BeautifulSoup(response.text,'html.parser');element=soup.select_one(css_selector)
...
def scrape_crypto_data_from_kraken(pair):
    ...
    try:
        ...
        opening_price = float(ticker_data['o'])
...
def scrape_tradingview_ytd_data(url,headers):
	try:
		logging.info(f"Fetching TradingView URL: {url}");response=requests.get(url,headers=headers,timeout=15);response.raise_for_status();soup=BeautifulSoup(response.text,'html.parser');selector='button[class*="selected"] span[class*="change-"]';value_element=soup.select_one(selector)
		if value_element:
...
def scrape_ecb_rate_data(url,headers):
	try:
		logging.info(f"Fetching ECB URL: {url}");response=requests.get(url,headers=headers,timeout=15);response.raise_for_status();soup=BeautifulSoup(response.text,'html.parser');data_cell=soup.select_one('td strong')
...
def scrape_indexa_data(headers):
	...
	try:
		logging.info(f"Fetching Indexa Capital URL: {url}");response=requests.get(url,headers=headers,timeout=15);response.raise_for_status();soup=BeautifulSoup(response.text,'html.parser');selector='td.border-left.js-plusmin';value_cell=soup.select_one(selector)
...
def get_bogle_quotes():
	...
	try:
		logging.info(f"Fetching Bogle quotes from: {BOGLE_QUOTES_URL}");response=requests.get(BOGLE_QUOTES_URL,headers=HEADERS,timeout=15);response.raise_for_status();soup=BeautifulSoup(response.text,'html.parser');quotes=[p.get_text(strip=True,separator=' ')for p in soup.select('div.thrv_tw_quote p')]
		if quotes:bogle_quotes_cache=quotes;return random.choice(bogle_quotes_cache)
		return'No invierta, simplemente, posea acciones.'
	except Exception as e:return'El error más grande es no permanecer en el camino.'
...
def get_all_instrument_data():
	try:
		...
		if 8<=current_hour_spain<15:
			future_perc=instrument_values_temp.get('sp500_futures',{}).get('percentage_float');usd_eur_perc=instrument_values_temp.get('usd_eur',{}).get('percentage_float')
			if future_perc is not None and usd_eur_perc is not None:
				predicted_opening=future_perc+usd_eur_perc;formatted_prediction=f"{predicted_opening:+.2f}%".replace('.',',');sp500_comment_text=f"S&P 500: Se prevé que abra sobre {formatted_prediction}."
		...
		world_perc=instrument_values_temp.get('world_net_eur',{}).get('percentage_float');world_comment_text,world_comment_sentiment='','neutral'
